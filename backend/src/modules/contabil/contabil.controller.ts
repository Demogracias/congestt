...
router.get('/contas', asyncHandler(async (req, res) => {
  const empresaId = (req.query as any).empresaId as string | undefined;
  const result = await contasService.listar(empresaId);
  res.json(result);
}));

router.get('/contas/:id', asyncHandler(async (req, res) => {
...
router.post('/contas/importar', asyncHandler(async (req: AuthRequest, res) => {
...
router.get('/grade', asyncHandler(async (req, res) => {
...
router.get('/fechamentos/:id', asyncHandler(async (req, res) => {
  const fech = await fechamentoService.buscarPorId(req.params.id);
  res.json(fech);
}));

router.get('/fechamentos/:id/historico', asyncHandler(async (req, res) => {
  const hist = await fechamentoService.historicoPorFechamento(req.params.id);
  res.json(hist);
}));

router.post('/fechamentos', asyncHandler(async (req: AuthRequest, res) => {
...
router.post('/fechamentos/:id/concluir', asyncHandler(async (req: AuthRequest, res) => {
  const { usuarioId } = req.body;
  const fech = await fechamentoService.concluir(req.params.id, usuarioId, req.usuario?.level || 0);
  res.json(fech);
}));

router.post('/fechamentos/:id/justificativa', asyncHandler(async (req: AuthRequest, res) => {
